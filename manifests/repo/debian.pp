class collectd::repo::debian {
  ...
  if $::collectd::ci_package_repo {
    ...
    if $::operatingsystem == 'Debian' {
      notify{'Youre trying to use the Ubuntu PPA on a Debian Server':}
      notify{'Recomend you use the ci_package_repo parameter if you with to use an upstream repo on Debian':}
    } else {
